...
int main(void) {
    ...
    for (;;) {
        ...
        switch(event_source) {
            ...
            /////////////////////////////////////////////
            case __event__(ull_to_double) : ull_to_double(); break;
...
/*void search_burst() {
    ...
    //Создание буфера для приема пакета
    uint64_t *buffer = (uint64_t*)malloc(size_in_bytes);
    //Выборка максимального ключа
    lnh_get_last(TEST_STRUCTURE);
...
void search_burst() {
    ...
    //Поиск по ключу
	auto f = lnh_ngr(TEST_STRUCTURE, key);
    mq_send(f);
    //Отправка ответа
